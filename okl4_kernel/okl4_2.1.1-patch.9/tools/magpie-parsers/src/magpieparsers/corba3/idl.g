...
specification
	...
	;
	exception
	catch [antlr.RecognitionException ex] {self.error_in_specification(ex)}
...
definition
	...
	;
	exception
	catch [antlr.RecognitionException ex] {self.error_in_definition(ex)}
...
scoped_name
	:   ( scope_operator )? identifier (scope_operator identifier)*
	;
...
integer_type
	: explicit_integer_sign
	| explicit_integer_sign base_integer_type
	| base_integer_type
	;
	//: (explicit_integer_sign)?
	//(base_integer_type)?
...
op_type_spec
	:   (ref_indirection)? param_type_spec (allow_indirection)?
	;
...
options {
  paraphrase = "a preprocessor directive";
	...
	:
	'#'!
	(txt:ALL_TO_NL) {self.preprocessor_directive(txt)}'\n'!
