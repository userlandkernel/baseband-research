                   ...
                   int line, const char * msg, ...)
{
  ...
  if (!result) {
    ...
    /* Unlike normal libcheck, we don't want to mark all the time, as it can waste a
       lot of memory, so we only mark on failure */
    send_loc_info (get_send_key(), file, line);
    ...
    if (cur_fork_status() == CK_FORK)
      exit(1);
    else if(cur_fork_status() == CK_WITHPAGER)
    {
      L4_MsgTag_t tag = L4_Niltag;
      // let the main thread know that we failed
      // Label of 1 for failure
      tag = L4_MsgTagAddLabel(tag, 1);
      L4_Set_MsgTag(tag);
      L4_Call(L4_Pager());
      // the main thread will kill us now
    }
  }
}
...
void srunner_exceptions(SRunner * sr, char * exceptions[])
{
    sr->exceptions = exceptions;
}
...
{
  if (fstat == CK_FORK || fstat == CK_NOFORK || fstat == CK_WITHPAGER)
    _fstat = fstat;
  else
