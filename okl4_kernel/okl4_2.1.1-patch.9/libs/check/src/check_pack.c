...
static int   read_buf   (FILE *f, char **buf);
...
void ppack (FILE *f, enum ck_msg_type type, CheckMsg *msg)
{
  char *buf;
  int n;
  size_t r;
...
static int read_buf (FILE *f, char **buf)
{
  ...
  *buf = emalloc(size);
  readloc = *buf;
  while (1) {
    n = fread (readloc, 1, size - nread, f);
    nread += n;
    if (n < size - nread) {
	    if (feof(f))
		    break;
...
static RcvMsg *rcvmsg_create (void)
{
  ...
  rmsg->lastctx = CK_CTX_UNSET;
  rmsg->failctx = CK_CTX_UNSET;
  rmsg->msg = NULL;
  reset_rcv_test (rmsg);
